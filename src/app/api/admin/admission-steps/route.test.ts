...
vi.mock("@/lib/db", () => ({ db: { admissionStep: h.delegate } }));
...
function makeReq({
  method = "GET",
  url = "http://localhost/api/admin/admission-steps",
  body,
  headers = {},
}: {
  method?: string;
...
const VALID = {
  ...
  description: "Complete the online application form with your child's details.",
  order: 1,
};
...
describe("POST create", () => {
  it("creates, invalidates cache, audits, returns 201", async () => {
    ...
    expect(h.delegate.create).toHaveBeenCalledWith({ data: expect.objectContaining({ title: "Submit Application" }) });
    expect(h.invalidateCache).toHaveBeenCalledWith("admissionSteps");
    expect(h.writeAuditLog).toHaveBeenCalledWith(
...
describe("item routes [id]", () => {
  ...
  it("PUT returns 400 on schema validation failure", async () => {
    const res = await PUT(makeReq({ method: "PUT", body: { step: "" } }), { params: { id: VALID_ID } });
    expect(res.status).toBe(400);
