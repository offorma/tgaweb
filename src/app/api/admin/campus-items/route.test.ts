...
vi.mock("@/lib/db", () => ({ db: { campusItem: h.delegate } }));
...
function makeReq({
  method = "GET",
  url = "http://localhost/api/admin/campus-items",
  body,
  headers = {},
}: {
  method?: string;
...
const VALID = {
  ...
  description: "A bright, well-stocked library at the heart of the campus.",
  order: 1,
};
...
describe("POST create", () => {
  it("creates, invalidates cache, audits, returns 201", async () => {
    ...
    expect(h.delegate.create).toHaveBeenCalledWith({ data: expect.objectContaining({ title: "Modern Library" }) });
    expect(h.invalidateCache).toHaveBeenCalledWith("campusItems");
    expect(h.writeAuditLog).toHaveBeenCalledWith(
...
describe("item routes [id]", () => {
  ...
  it("PUT returns 400 on schema validation failure", async () => {
    const res = await PUT(makeReq({ method: "PUT", body: { title: "" } }), { params: { id: VALID_ID } });
    expect(res.status).toBe(400);
