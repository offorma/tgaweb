...
vi.mock("@/lib/db", () => ({ db: { download: h.delegate } }));
...
function makeReq({
  method = "GET",
  url = "http://localhost/api/admin/downloads",
  body,
  headers = {},
}: {
  method?: string;
...
const VALID = {
  ...
  description: "Our full school prospectus",
...
describe("POST create", () => {
  it("creates, invalidates cache, audits, returns 201", async () => {
    ...
    expect(h.delegate.create).toHaveBeenCalledWith({ data: expect.objectContaining({ name: "Prospectus 2026" }) });
    expect(h.invalidateCache).toHaveBeenCalledWith("downloads");
    expect(h.writeAuditLog).toHaveBeenCalledWith(
...
describe("item routes [id]", () => {
  ...
  it("PUT returns 400 on schema validation failure", async () => {
    const res = await PUT(makeReq({ method: "PUT", body: { url: "not a url at all !!" } }), { params: { id: VALID_ID } });
    expect(res.status).toBe(400);
