...
vi.mock("@/lib/db", () => ({ db: { faq: h.delegate } }));
...
function makeReq({
  method = "GET",
  url = "http://localhost/api/admin/faqs",
  body,
  headers = {},
}: {
  method?: string;
...
const VALID = {
  ...
  answer: "The school day runs from 8:00am to 3:30pm, Monday through Friday.",
  order: 1,
};
...
describe("POST create", () => {
  it("creates, invalidates cache, audits, returns 201", async () => {
    ...
    expect(h.delegate.create).toHaveBeenCalledWith({ data: expect.objectContaining({ question: "What are the school hours?" }) });
    expect(h.invalidateCache).toHaveBeenCalledWith("faqs");
    expect(h.writeAuditLog).toHaveBeenCalledWith(
...
describe("item routes [id]", () => {
  ...
  it("PUT returns 400 on schema validation failure", async () => {
    const res = await PUT(makeReq({ method: "PUT", body: { question: "" } }), { params: { id: VALID_ID } });
    expect(res.status).toBe(400);
