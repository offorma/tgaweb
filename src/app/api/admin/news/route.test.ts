...
vi.mock("@/lib/db", () => ({ db: { newsItem: h.delegate } }));
...
function makeReq({
  method = "GET",
  url = "http://localhost/api/admin/news",
  body,
  headers = {},
}: {
  method?: string;
...
const VALID = {
  ...
  category: "News",
  tag: "Achievement",
  title: "Pupils win national science award",
  excerpt: "Our Year 6 team took first place at the national young scientists competition.",
...
describe("GET list", () => {
  it("returns the items ordered", async () => {
    h.delegate.findMany.mockResolvedValue([{ id: VALID_ID }]);
    const res = await GET(makeReq(), {});
...
describe("POST create", () => {
  it("creates, invalidates cache, audits, returns 201", async () => {
    ...
    expect(h.delegate.create).toHaveBeenCalledWith({ data: expect.objectContaining({ title: "Pupils win national science award" }) });
    expect(h.invalidateCache).toHaveBeenCalledWith("news");
    expect(h.writeAuditLog).toHaveBeenCalledWith(
  ...
  it("returns 400 on schema validation failure", async () => {
    const res = await POST(makeReq({ method: "POST", body: { __bad: true } }), {});
...
describe("item routes [id]", () => {
  ...
  it("GET returns the item when found (Promise params)", async () => {
    h.delegate.findUnique.mockResolvedValue({ id: VALID_ID });
    const res = await GET_ONE(makeReq(), { params: Promise.resolve({ id: VALID_ID }) });
    expect(res.status).toBe(200);
    expect((await res.json()).item.id).toBe(VALID_ID);
  });
  ...
  it("PUT returns 400 on schema validation failure", async () => {
    const res = await PUT(makeReq({ method: "PUT", body: { date: "not-a-date" } }), { params: { id: VALID_ID } });
    expect(res.status).toBe(400);
