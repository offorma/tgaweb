...
vi.mock("@/lib/db", () => ({ db: { program: h.delegate } }));
...
function makeReq({
  method = "GET",
  url = "http://localhost/api/admin/programs",
  body,
  headers = {},
}: {
  method?: string;
...
const VALID = {
  name: "Early Years",
  ages: "3-5 years",
  image: "https://example.com/ey.jpg",
  color: "orange",
  tagline: "Where curiosity begins",
  description: "A nurturing start built on play-based discovery and foundational skills.",
  features: "Play-based learning\nQualified staff\nSafe environment",
  order: 1,
};
...
describe("POST create", () => {
  it("creates, invalidates cache, audits, returns 201", async () => {
    ...
    expect(h.delegate.create).toHaveBeenCalledWith({ data: expect.objectContaining({ name: "Early Years" }) });
    expect(h.invalidateCache).toHaveBeenCalledWith("programs");
    expect(h.writeAuditLog).toHaveBeenCalledWith(
...
describe("item routes [id]", () => {
  ...
  it("PUT returns 400 on schema validation failure", async () => {
    const res = await PUT(makeReq({ method: "PUT", body: { color: "purple" } }), { params: { id: VALID_ID } });
    expect(res.status).toBe(400);
