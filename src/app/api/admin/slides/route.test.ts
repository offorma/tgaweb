...
vi.mock("@/lib/db", () => ({ db: { slide: h.delegate } }));
...
function makeReq({
  method = "GET",
  url = "http://localhost/api/admin/slides",
  body,
  headers = {},
}: {
  method?: string;
...
describe("POST create", () => {
  it("creates, invalidates cache, audits, returns 201", async () => {
    ...
    expect(h.delegate.create).toHaveBeenCalledWith({ data: expect.objectContaining({ title: "Welcome to our school" }) });
...
describe("item routes [id]", () => {
  ...
  it("PUT returns 400 on schema validation failure", async () => {
    const res = await PUT(makeReq({ method: "PUT", body: { duration: 100 } }), { params: { id: VALID_ID } });
    expect(res.status).toBe(400);
