...
vi.mock("@/lib/db", () => ({ db: { stat: h.delegate } }));
...
const VALID = {
  ...
  suffix: "+",
  label: "Happy Pupils",
  description: "Enrolled across all year groups",
  order: 1,
};
...
describe("POST create", () => {
  it("creates, invalidates cache, audits, returns 201", async () => {
    ...
    expect(h.delegate.create).toHaveBeenCalledWith({ data: expect.objectContaining({ label: "Happy Pupils" }) });
    expect(h.invalidateCache).toHaveBeenCalledWith("stats");
    expect(h.writeAuditLog).toHaveBeenCalledWith(
...
describe("item routes [id]", () => {
  ...
  it("PUT returns 400 on schema validation failure", async () => {
    const res = await PUT(makeReq({ method: "PUT", body: { value: -5 } }), { params: { id: VALID_ID } });
    expect(res.status).toBe(400);
