...
vi.mock("@/lib/db", () => ({ db: { value: h.delegate } }));
...
function makeReq({
  method = "GET",
  url = "http://localhost/api/admin/values",
  body,
  headers = {},
}: {
  method?: string;
...
const VALID = {
  icon: "Heart",
  title: "Compassion",
  description: "We nurture kindness and empathy in every interaction across the school.",
  order: 1,
};
...
describe("POST create", () => {
  it("creates, invalidates cache, audits, returns 201", async () => {
    ...
    expect(h.delegate.create).toHaveBeenCalledWith({ data: expect.objectContaining({ title: "Compassion" }) });
    expect(h.invalidateCache).toHaveBeenCalledWith("values");
    expect(h.writeAuditLog).toHaveBeenCalledWith(
